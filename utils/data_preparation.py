...
floor_list = []
for it, row in df[['piano', 'totale piani edificio']].iterrows():
    floor_dict = {'piano': None, 'delta_piano': None, 'ascensore': None}
    floor_str = row['piano'].lower()
    tot_floor = row['totale piani edificio']
    try:
        tot_floor = int(re.findall('\d', tot_floor)[0])
    except TypeError:
        continue
    if "ascensore" in floor_str:
        floor_dict['ascensore'] = 1
    matches = re.findall('\d°', floor_str)
    if len(matches) >= 1:
        floor_dict['piano'] = int(matches[0][0])
        floor_dict['delta_piano'] = tot_floor - floor_dict['piano']
    elif len(matches) == 0:
        if "ultimo" in floor_str:
            floor_dict['piano'] = tot_floor
            floor_dict['delta_piano'] = 0
        elif "terra" in floor_str or "rialzato" in floor_str or "ammezzato" in floor_str:
            floor_dict['piano'] = 0
            floor_dict['delta_piano'] = tot_floor
        elif "seminterrato" in floor_str:
            floor_dict['piano'] = -1
            floor_dict['delta_piano'] = tot_floor + 1  # to rethink
        else:
            pass
...
df.to_csv("immobiliare_ads_clean_2023-01-07.csv", index=False)
